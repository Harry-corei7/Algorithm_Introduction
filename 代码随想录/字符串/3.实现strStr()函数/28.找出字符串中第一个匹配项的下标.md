        ...
        for i in range(len(haystack)):
            if(haystack[i] == needle[0]):
                for j in range(i, i+len(needle)):
                    if(len(haystack) < i+len(needle)): # 如果剩下的haystack部分比needle短，直接return-1
                        return -1
                    if(haystack[j] != needle[j - i]): # 如果对比过程中有不符，直接结束这次循环判断
                        break
                    if(j == i+len(needle)-1): # 如果全部判断完，则return index
                        index = i
                        return index
        return -1
```
