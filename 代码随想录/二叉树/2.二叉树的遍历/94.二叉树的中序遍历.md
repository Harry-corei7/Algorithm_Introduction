...
class Solution(object):
    def postorderTraversal(self, root):
        ...
        stack = [] # 设置栈用于遍历
        res = [] # 设置答案数组
        if(root == None): return res # 若为空，则返回空数组
        stack.append(root)
        while(len(stack) != 0):
            # 左右中，所以中右左
            node = stack.pop(-1)
...
class Solution(object):
    def inorderTraversal(self, root):
        ...
        # 与前序遍历不同，"左中右"需要一个指针cur，以便从root到达最左端left
        cur = root
        while(cur != None or len(stack) != 0):
            # 左中右
            if(cur != None): # 先迭代访问最底层的左子树节点
                stack.append(cur)
                cur = cur.left
            else: # 当cur == None, 后处理栈顶节点(最左节点)
                cur = stack.pop(-1) # cur指向最左节点
                res.append(cur.val)
                cur = cur.right # 指向"当前"节点的右节点，入栈后先处理
        return res
```
