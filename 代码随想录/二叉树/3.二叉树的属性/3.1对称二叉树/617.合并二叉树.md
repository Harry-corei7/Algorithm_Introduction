...
node_1 = TreeNode(5)   # 问题在这里！对作为None的node_1进行修改，无法改变原树的结构
```
---
`python语法`
...
class Solution(object):
    def mergeTrees(self, root1, root2):
        ...
        while(len(queue) != 0):
            ...
            # 对左节点进行入队列处理
            if((node_1.left == None) and (node_2.left != None)): # 不对称，补充
                node_1.left = TreeNode(0)
            if((node_1.left != None) and (node_2.left == None)): # 不对称，补充
                node_2.left = TreeNode(0)
            if((node_1.left != None) and (node_2.left != None)): # 统一入队列，下轮进行相加
                queue.append(node_1.left)
                queue.append(node_2.left)
            
            # 对右节点进行入队列处理
            if((node_1.right == None) and (node_2.right != None)): # 不对称，补充
                node_1.right = TreeNode(0)
            if((node_1.right != None) and (node_2.right == None)): # 不对称，补充
                node_2.right = TreeNode(0)
            if((node_1.right != None) and (node_2.right != None)): # 统一入队列，下轮进行相加
...
```python
# 错误做法示例: 在每一轮对'当前节点'进行处理，若'当前节点'为None就束手无策
class Solution(object):
    def mergeTrees(self, root1, root2):
        """
        :type root1: Optional[TreeNode]
