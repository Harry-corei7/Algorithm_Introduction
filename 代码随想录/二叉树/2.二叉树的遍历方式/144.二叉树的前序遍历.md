...
class Solution(object):
    def preorderTraversal(self, root):
        ...
        stack = [] # 设置栈用于遍历
        res = [] # 设置res作为结果列表
        if(root == None): return res # 若为空节点，直接返回None
        stack.append(root) # 根节点入栈
        while(len(stack) != 0):
            node = stack.pop(-1) # 获取栈最后一个节点作为"当前节点"
            res.append(node.val) # 中节点val入res数组
            # 中左右
            if(node.right != None): stack.append(node.right) # 右节点先入栈，后出
            if(node.left != None): stack.append(node.left) # 左节点后入栈，先出
        return res
```
