...
class Solution(object):
    ...
    def jump(self, nums):
        ...
        return step
```
`python语法：贪心算法(个人理解+step_container数组版)`
```python
class Solution(object):
    def Jump(self, nums, i, step_container):
        cover = nums[i]
        if(i+cover >= len(nums)-1): 
            step_container[0] += 1
            return # 直接return结束递归
        ...
        # step+1表示跳跃到maxindex
        step_container[0] += 1
        self.Jump(nums, max_index, step_container)
    ...
    def jump(self, nums):
        ...
        if(len(nums) <= 1): return 0
        step_container = []
        step_container.append(0)
        self.Jump(nums, 0, step_container)
        return step_container[0]
```
