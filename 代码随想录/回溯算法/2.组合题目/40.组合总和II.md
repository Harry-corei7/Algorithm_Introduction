...
```python
class Solution(object):
    def backtracking(self, candidates, target, result, path, startIndex, total):
        if(total == target):
            result.append(path[:])
            return
        for i in range(startIndex, len(candidates)):
            if(i > startIndex and (candidates[i] == candidates[i-1])): # 剪枝(1): 排序后，将同一层的相同元素去除
                continue
            
            if(total + candidates[i] > target): # 剪枝(2): 设置total变量储存sum，不用每次计算
                break

            path.append(candidates[i]) # append后，total也需要+=
            total += candidates[i]
            self.backtracking(candidates, target, result, path, i+1, total)
            total -= candidates[i]
            path.pop(-1)
    ...
    def combinationSum2(self, candidates, target):
        ...
        result = []
        path = []
        self.backtracking(sorted(candidates), target, result, path, 0, 0)
        return result
```
