...
`2) 慢指针(slowIndex)在新的result数组慢慢填充(或者用for循环的i指针也行), 而不是在原数组覆盖`
...
class Solution {
    public int[] sortedSquares(int[] nums) {
        // 双指针 - 左右指针法
        // (1) 本质是三指针: 左右指针在原数组相互逼近
        // (2) 慢指针(slowIndex)在新的result数组慢慢填充, 而不是在原数组覆盖
...
class Solution(object):
    def sortedSquares(self, nums):
        ...
        :rtype: List[int]
        """
        res = [0] * len(nums) # 初始化一个固定长度的数组
        right = len(nums) - 1 
        left = 0
        slowIndex = len(nums) - 1
        while(left <= right):
            if(nums[left] + nums[right] >= 0):
                res[slowIndex] = nums[right]**2
                right-=1
            elif(nums[left] + nums[right] < 0):
                res[slowIndex] = nums[left]**2
                left+=1
            slowIndex-=1
        return res
```
